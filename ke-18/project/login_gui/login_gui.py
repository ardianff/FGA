...
class LoginRegisterUser:
    ...
    def register_user(self) :
        ...
        file = open('D:\Kuliah\FGA\ke-18\project_login\login_gui\database.txt',"a")
        file.write("\n"+get_name+","+get_username+","+get_password)
        file.close()
        self.entryUserName.delete(0, END)
        self.entryUser.delete(0, END)
        self.entryPass.delete(0, END)
        Label(screen1, text="Registration Success", fg="green", font=("calibri", 11)).pack(side = BOTTOM)
            
    def do_login(self):
        get_username = self.entryUser.get()
        get_password = self.entryPass.get()
        sukses = False
        file = open('D:\Kuliah\FGA\ke-18\project_login\login_gui\database.txt',"r")
        for i in file:
